    ...
    if (isset($_POST['Token']) && !empty($_POST['Token'])){
        ...
        if (mysqli_num_rows($select_token)==1){
            ...
            if (isset($_POST["Cat-id"]) &&
                ...
                $cat_name = $functions->real_escape($_POST["Cat_Name"]);
                $pic_name = $functions->random_char(18);
                $upload_image = $functions->upload_img_multipart($_FILES['Image_Up'],'../../Images/',$pic_name);

                if ($upload_image){

                    $insert_subcat = mysqli_query($functions->connection(),
                        "INSERT INTO subcat_food (id, cat_id, name, pic, reg_date, reg_time) VALUES ('$id','$cat_id','$cat_name','$pic_name','$reg_date','$reg_time')");

                    if ($insert_subcat){
                        echo json_encode(array("Erorr"=>false,"MSG"=>"زیر دسته بندی مورد نظر با موفقیت ثبت شد !"));
